...
const Index = ({ unreadChats }) => {
	const [search, setSearch] = useState('');
	// eslint-disable-next-line no-unused-vars
	const [chatOpened, setChatOpened] = useRecoilState(rChatOpened);
	const [displayPatients, setDisplayPatients] = useState(unreadChats ?? []);

	const filter = () => {
		let patientsFiltered = unreadChats;
		if (search !== '') {
			patientsFiltered = unreadChats.filter(
				person =>
